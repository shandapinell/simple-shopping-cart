    ...
    <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
      <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
        <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
          <div className="sm:flex sm:items-start">
            <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
              <h3 className="text-base font-semibold text-gray-900" id="modal-title">Cart ({cart.length})</h3>
              {
                cart.length > 0 ? (
                  <div className="mt-2 w-full">
                        {cart.map((product) => (
                            <div key={product.id} className="flex items-center justify-between">
                                ...
                                <button className="mb-2" onClick={() => removeFromCart(product)}>Remove</button>
                            </div>
                        ))}
                  </div>
                ) : (
        ...
        <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-4">
          ...
          <button disabled={cart.length === 0} type="button" className="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto" onClick={closeCart}>Close</button>
        </div>
      </div>
    </div>
  </div>
